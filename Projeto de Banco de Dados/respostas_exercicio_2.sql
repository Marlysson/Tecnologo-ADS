...
SELECT NOME_CAT FROM CATEGORIA NATURAL JOIN APARTAMENTO 
	GROUP BY COD_CAT 
	HAVING COUNT(COD_CAT) = 1
...
SELECT NOME , EXTRACT(MONTH FROM DT_NASC) "Mês de nascimento" , EXTRACT(YEAR FROM DT_NASC) "Ano de nascimento" FROM HOSPEDE
	WHERE UPPER(NACIONALIDADE) LIKE 'BRASILEIRO'
...
(SELECT NOME FROM HOSPEDE
	...
	NATURAL JOIN FUNCIONARIO
	NATURAL JOIN CATEGORIA
	WHERE UPPER(NOME) LIKE 'JOÃO%'
	OR UPPER(NOME_CAT) LIKE 'LUXO%'
UNION
SELECT NOME FROM RESERVA
	NATURAL JOIN FUNCIONARIO
	NATURAL JOIN CATEGORIA
	WHERE COD_HOSP IN (SELECT COD_HOSP FROM HOSPEDE WHERE UPPER(NOME) LIKE 'JOÃO%')
	OR UPPER(NOME_CAT) LIKE 'LUXO') ORDER BY NOME ASC

-- QUESTÃO 15: O código das hospedagens realizadas pelo hóspede mais velho dentre os que se hospedagem no apartamento mais caro
...
SELECT H1.NOME FROM HOSPEDE H1 , HOSPEDE H2 WHERE 
	H1.COD_HOSP <> H2.COD_HOSP 
	AND H1.DT_NASC = H2.DT_NASC
	AND H2.COD_HOSP = 2
...
(SELECT MIN(DT_NASC) FROM HOSPEDAGEM 
	NATURAL JOIN HOSPEDE
	NATURAL JOIN APARTAMENTO
	NATURAL JOIN CATEGORIA
	WHERE EXTRACT(YEAR FROM DT_ENTRADA) = 2017
	AND VALOR_DIA = (SELECT MAX(VALOR_DIA) FROM CATEGORIA))
...
SELECT NOME_CAT FROM RESERVA
	NATURAL JOIN HOSPEDE
	NATURAL JOIN CATEGORIA
	WHERE UPPER(NOME) LIKE 'MARIA%'
UNION
SELECT NOME_CAT FROM HOSPEDE
	NATURAL JOIN HOSPEDAGEM
	NATURAL JOIN APARTAMENTO
	NATURAL JOIN CATEGORIA
	WHERE UPPER(HOSPEDE.NOME) LIKE 'JOÃO%'
